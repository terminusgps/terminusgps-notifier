...
async def select_resources(request: HttpRequest) -> HttpResponse:
    context = {}
    try:
        user = await request.auser()
        customer, _ = await Customer.objects.aget_or_create(user=user)
...
@require_GET
@htmx_template("terminusgps_notifier/select_triggers.html")
async def select_triggers(request: HttpRequest) -> HttpResponse:
    context = {"options": forms.WialonNotificationTrigger.choices}
    return render(request, request.template_name, context=context)
